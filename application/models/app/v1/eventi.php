...
class Eventi extends CI_Model {
  ...
  function __construct(){
    ...
    $this->load->model("app/v1/iscrizioni");
}
...
public function get_faq_evento($id_evento){
    ...
    return "http://www.performancestrategies.it";
}
...
public function list_eventi($azienda, $id_iscrizione = NULL){        
    $this->db1->select("E.evento, A.*, D.id as id_dettaglio_evento");
    $this->db1->from("crm_eventi_def E");
    ...
    if ($query->num_rows() > 0) {
      ...
      foreach ($query->result() as $row) {
        ...
        if($now > $data_start && $now < $data_stop){
            $bool_login = $this->iscrizioni->getIscrizione($row->id_dettaglio_evento, $id_iscrizione);
            $text_btn = "Live 2.0";
            $titolo_card = "";
            $flag_test = false;
            $id_evento_live = $row->id_dettaglio_evento;
        }
